...
const ReportSchema = z.object({
  ...
  repasse: z.string().min(1, "O campo Repasse é obrigatório."),
...
export async function fetchUrlContent(
  ...
  const validatedFields = ReportSchema.safeParse({
    ...
    parceiro: formData.get("parceiro"),
  });
  ...
  const { cookie, repasse, parceiro } = validatedFields.data;

  if (!cookie) {
      return { success: false, error: "Autenticação necessária. Por favor, obtenha um cookie primeiro." };
  }
  ...
  body.append('tipo_impressão', 'html'); // <-- Alterado de 'xls' para 'html'
  ...
  try {
    ...
    if (!response.ok) {
      return { success: false, error: `A requisição falhou com o status: ${response.status} ${response.statusText}` };
    }

    const contentType = response.headers.get("content-type");
    const buffer = await response.arrayBuffer();
    
    // Decodifica como texto (usando latin1 para evitar perda de dados)
    const responseBody = Buffer.from(buffer).toString('latin1');
    
    return { success: true, html: responseBody, contentType };
  } catch (e: any) {
    console.error("Erro na Action fetchUrlContent:", e);
