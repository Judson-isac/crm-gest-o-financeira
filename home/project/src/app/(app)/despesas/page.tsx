...
import { getDistinctValues, getDespesas } from "@/lib/api";
  ...
  const fetchData = useCallback(() => {
    if (selectedAno && selectedMes) {
      startDataLoading(async () => {
        const allDespesas = await getDespesas({ referencia_ano: selectedAno, referencia_mes: selectedMes });
        const { polos } = await getDistinctValues();

        const despesasMap = polos.reduce((acc, polo) => {
            acc[polo] = 0;
            return acc;
        ...
        const data = Object.entries(despesasMap).map(([polo, total]) => ({ polo, total })).sort((a, b) => a.polo.localeCompare(b.polo));
