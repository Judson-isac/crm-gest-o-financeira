...
export async function getDespesas(filters: { polo?: string | string[], referencia_ano?: number, referencia_mes?: number }): Promise<Despesa[]> {
    ...
    // Despesas are not tied to a network in the current schema
    return db.getDespesas({ ...filters, polo: effectivePoloFilter });
}
...
export async function getCursos(): Promise<Curso[]> { 
    const { isSuperAdmin, sessionRedeId } = await getApiContext();
    if (isSuperAdmin) {
        // Superadmin should maybe see all? Or filter by a network?
        // For now, let's say they see all courses if no specific network is requested.
        return db.getCursos();
    }
    if (sessionRedeId) {
...
export async function getMetas(): Promise<Meta[]> { return db.getMetas(); }
export async function getSpacepoints(): Promise<Spacepoint[]> { return db.getSpacepoints(); }
export async function getTiposCurso(): Promise<TipoCurso[]> { return db.getTiposCurso(); }
