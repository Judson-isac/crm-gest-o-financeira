"use client";
import { useState } from "react";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Legend } from "recharts";
import { formatCurrency } from "@/lib/utils";
import { Move, X } from "lucide-react";
import { Button } from "@/components/ui/button";
import type { LucratividadePorCursoData } from "@/lib/types";
import { ChartLayoutSwitcher, ChartLayout } from "./chart-layout-switcher";
import { useDashboard } from "@/contexts/DashboardContext";

type WidgetProps = {
  data: LucratividadePorCursoData[];
  onRemove?: () => void;
};

export function LucroCursoEADChart({ data, onRemove }: WidgetProps) {
    const { palette } = useDashboard();
    const [barLayout, setBarLayout] = useState<ChartLayout>('vertical');
    
    const chartData = data
      .filter(item => item.metodologia === 'EAD')
      .map(item => ({
        nome: item.nome,
        'Repasse': item.repasse,
        'Despesa Direta': item.despesa,
        'Lucro': item.lucro,
    }));
  
    const renderChart = () => {
      if (barLayout === 'vertical') {
        return (
            <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 5, right: 20, left: 20, bottom: 80 }}>
                    <XAxis type="category" dataKey="nome" fontSize={12} interval={0} angle={-45} textAnchor="end" height={80} />
                    <YAxis type="number" tickFormatter={(value) => `${formatCurrency(Number(value) / 1000)}k`} fontSize={12} />
                    <Tooltip cursor={{ fill: "hsl(var(--muted))" }} formatter={(value: number) => formatCurrency(value)} />
                    <Legend wrapperStyle={{ fontSize: "12px", paddingTop: '20px' }} />
                    <Bar dataKey="Repasse" fill={palette[0]} radius={[4, 4, 0, 0]} />
                    <Bar dataKey="Despesa Direta" fill={palette[3]} radius={[4, 4, 0, 0]} />
                    <Bar dataKey="Lucro" fill={palette[1]} radius={[4, 4, 0, 0]} />
                </BarChart>
            </ResponsiveContainer>
        );
      } else { // horizontal
          const dynamicHeight = Math.max(300, chartData.length * 60);
          return (
              <div style={{ height: `${dynamicHeight}px` }}>
                  <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 30, left: 150, bottom: 5 }}>
                          <XAxis type="number" tickFormatter={(value) => `${formatCurrency(Number(value) / 1000)}k`} fontSize={12} />
                          <YAxis type="category" dataKey="nome" width={150} tickLine={false} axisLine={false} fontSize={12} interval={0} />
                          <Tooltip cursor={{ fill: "hsl(var(--muted))" }} formatter={(value: number) => formatCurrency(value)} />
                          <Legend wrapperStyle={{ fontSize: "12px" }} />
                           <Bar dataKey="Repasse" fill={palette[0]} radius={[0, 4, 4, 0]} />
                          <Bar dataKey="Despesa Direta" fill={palette[3]} radius={[0, 4, 4, 0]} />
                          <Bar dataKey="Lucro" fill={palette[1]} radius={[0, 4, 4, 0]} />
                      </BarChart>
                  </ResponsiveContainer>
              </div>
          );
      }
    }

  return (
    <Card className="h-full grid grid-rows-[auto_1fr]">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div className="drag-handle cursor-move">
            <CardTitle className="text-base">Gráfico: Lucro por Curso EAD</CardTitle>
            <CardDescription className="text-xs">Repasse vs. Despesa Direta por Curso</CardDescription>
        </div>
        <div className="flex items-center gap-2">
             <ChartLayoutSwitcher value={barLayout} onChange={setBarLayout} />
            {onRemove && (
                <Button variant="ghost" size="icon" className="h-8 w-8" onClick={onRemove}>
                    <X className="h-4 w-4 text-muted-foreground" />
                </Button>
            )}
            <Move className="h-4 w-4 text-muted-foreground drag-handle cursor-move" />
        </div>
      </CardHeader>
      <CardContent className="p-4 grid grid-rows-1" style={{gridTemplateRows: 'minmax(0, 1fr)', overflow: 'auto'}}>
        {renderChart()}
      </CardContent>
    </Card>
  );
}
