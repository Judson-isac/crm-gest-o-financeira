...
export function EADLucroChart({ data, onRemove }: WidgetProps) {
    const { palette } = useDashboard();
    const [chartType, setChartType] = useState<ChartType>("bar");
    const [barLayout, setBarLayout] = useState<ChartLayout>('vertical');
    
    const chartData = data
      .filter(item => item.metodologia === 'EAD')
      .map(item => ({
        polo: item.polo,
        'Repasse EAD': item.repasse,
        'Despesa (Alocada)': item.despesa,
        'Lucro EAD': item.lucro,
    }));
  
    const renderChart = () => {
      // Functions to render vertical and horizontal bar charts
      const renderVerticalBarChart = () => (
          <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 5, right: 20, left: 20, bottom: 80 }}>
                  ...
                  <Bar dataKey="Repasse EAD" fill={palette[0]} radius={[4, 4, 0, 0]} />
                  <Bar dataKey="Despesa (Alocada)" fill={palette[1]} radius={[4, 4, 0, 0]} />
              </BarChart>
          </ResponsiveContainer>
      );
              ...
              <div style={{ height: `${dynamicHeight}px` }}>
                  <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 30, left: 100, bottom: 5 }}>
                          ...
                          <Bar dataKey="Repasse EAD" fill={palette[0]} radius={[0, 4, 4, 0]} />
                          <Bar dataKey="Despesa (Alocada)" fill={palette[1]} radius={[0, 4, 4, 0]} />
                      </BarChart>
                  </ResponsiveContainer>
              </div>
          );
            ...
            return (
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartData} margin={{ top: 5, right: 20, left: 20, bottom: 80 }}>
                        ...
                        <Tooltip cursor={{ fill: "hsl(var(--muted))" }} formatter={(value: number) => formatCurrency(value)} />
                        <Legend wrapperStyle={{ fontSize: "12px", paddingTop: '20px' }} />
                        <Line type="monotone" dataKey="Repasse EAD" stroke={palette[0]} />
                        <Line type="monotone" dataKey="Despesa (Alocada)" stroke={palette[1]} />
                        <Line type="monotone" dataKey="Lucro EAD" stroke={palette[2]} strokeDasharray="5 5" />
                    </LineChart>
                </ResponsiveContainer>
            );
        case 'area':
             return (
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData} margin={{ top: 5, right: 20, left: 20, bottom: 80 }}>
                        ...
                        <Tooltip cursor={{ fill: "hsl(var(--muted))" }} formatter={(value: number) => formatCurrency(value)} />
                        <Legend wrapperStyle={{ fontSize: "12px", paddingTop: '20px' }} />
                        <Area type="monotone" dataKey="Repasse EAD" stackId="a" stroke={palette[0]} fill={palette[0]} />
                        <Area type="monotone" dataKey="Despesa (Alocada)" stackId="a" stroke={palette[1]} fill={palette[1]} />
                    </AreaChart>
                </ResponsiveContainer>
            );
        case 'bar':
    ...
    <Card className="h-full grid grid-rows-[auto_1fr]">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div className="drag-handle cursor-move">
            <CardTitle className="text-base">Gráfico: Lucro Cursos EAD</CardTitle>
            <CardDescription className="text-xs">Repasse EAD vs. Despesa Proporcional</CardDescription>
        </div>
