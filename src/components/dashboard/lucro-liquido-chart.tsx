...
export function LucroLiquidoChart({ data, onRemove }: WidgetProps) {
      ...
      const renderVerticalBarChart = () => (
          <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 5, right: 20, left: 20, bottom: 80 }}>
                  ...
                  <Tooltip cursor={{ fill: "hsl(var(--muted))" }} formatter={(value, name) => [formatCurrency(Number(value)), name]} />
                  <Legend wrapperStyle={{ fontSize: "12px", paddingTop: '20px' }} />
                  <Bar dataKey="Repasse" fill={palette[0]} radius={[4, 4, 0, 0]} />
                  <Bar dataKey="Despesas" fill={palette[1]} radius={[4, 4, 0, 0]} />
              </BarChart>
          </ResponsiveContainer>
      );
              ...
              <div style={{ height: `${dynamicHeight}px` }}>
                  <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 30, left: 100, bottom: 5 }}>
                          ...
                          <Bar dataKey="Repasse" fill={palette[0]} radius={[0, 4, 4, 0]} />
                          <Bar dataKey="Despesas" fill={palette[1]} radius={[0, 4, 4, 0]} />
                      </BarChart>
                  </ResponsiveContainer>
              </div>
          );
            ...
            return (
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartData} margin={{ top: 5, right: 20, left: 20, bottom: 80 }}>
                        ...
                        <Legend wrapperStyle={{ fontSize: "12px", paddingTop: '20px' }} />
                        <Line type="monotone" dataKey="Repasse" stroke={palette[0]} />
                        <Line type="monotone" dataKey="Despesas" stroke={palette[1]} />
                        <Line type="monotone" dataKey="Lucro Líquido" stroke={palette[2]} strokeDasharray="5 5" />
                    </LineChart>
                </ResponsiveContainer>
            );
        case 'area':
            return (
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData} margin={{ top: 5, right: 20, left: 20, bottom: 80 }}>
                        ...
                        <Tooltip cursor={{ fill: "hsl(var(--muted))" }} formatter={(value) => formatCurrency(Number(value))} />
                        <Legend wrapperStyle={{ fontSize: "12px", paddingTop: '20px' }} />
                        <Area type="monotone" dataKey="Repasse" stackId="a" stroke={palette[0]} fill={palette[0]} />
                        <Area type="monotone" dataKey="Despesas" stackId="a" stroke={palette[1]} fill={palette[1]} />
                    </AreaChart>
                </ResponsiveContainer>
            );
        case 'bar':
    ...
    <Card className="h-full grid grid-rows-[auto_1fr]">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div className="drag-handle cursor-move">
            <CardTitle className="text-base">Gráfico: Lucro Líquido (Geral)</CardTitle>
            <CardDescription className="text-xs">Repasse Total vs. Despesa Total</CardDescription>
        </div>
