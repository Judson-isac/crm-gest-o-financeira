...
import { CourseImportHistoryTable } from '@/components/cursos/course-import-history-table';
...
export default function ImportacaoCursosPage() {
  ...
  const [isLoadingData, startDataLoadingTransition] = useTransition();
  ...
  const handleImport = () => {
    ...
    startImporting(async () => {
        ...
        if (result.success) {
            ...
            fetchData(); // Refresh data in tables
        } else {
    ...
    <Tabs defaultValue="importar" className="w-full">
      ...
      <TabsContent value="importar" className="mt-6">
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>Importar Arquivo de Cursos</CardTitle>
              <CardDescription>
                Use o método automático para buscar os dados mais recentes da Unicesumar ou o manual para subir um arquivo JSON.
              </CardDescription>
            </CardHeader>
            <CardContent>
                <Tabs defaultValue="automatico" className="w-full">
                    <TabsList className="grid w-full grid-cols-2">
                        <TabsTrigger value="automatico">Automático (Recomendado)</TabsTrigger>
                    ...
                    <TabsContent value="automatico" className="mt-4">
                        <div className="flex items-center gap-4">
                            <Button onClick={handleFetch} disabled={isProcessing}>
                                ...
                                {isFetching ? 'Buscando...' : 'Buscar Dados da Sede'}
                            </Button>
                        </div>
                    </TabsContent>
      ...
      <TabsContent value="historico" className="mt-6">
        <CourseImportHistoryTable imports={imports} onImportDeleted={fetchData} />
      </TabsContent>
    </Tabs>
  );
}
